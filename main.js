...
let admin = new adminDetail("Alishba wajahat", "alishbawajahat867@gmail.com", "1234");
let users = [];
console.log(admin);
class Event {
    eventName;
    ...
    noOfTickets;
    priceOfTicket;
    constructor(eventName, venue, timings, date, noOfTickets, priceOfTicket) {
    ...
    cancelTickets(ticketsToCancel) {
        if (ticketsToCancel > this.boughtTickets) {
            console.log(`You have only ${this.boughtTickets} tickets & you are giving a greater number!`);
        }
            ...
            console.log(`You've cancelled ${ticketsToCancel} tickets & and now ${this.boughtTickets} are left!`);
...
let eventsList = [];
function userExists(username, email, password) {
...
function isAdmin(username, email, password) {
    if ((username === admin.fullName && email === admin.email) && password === admin.password) {
        return true;
    }
    return false;
}
...
while (condition) {
    ...
    if (question0.Move === "Create an account-SIGNUP") {
        ...
        var user1 = new userDetail(question1.Name, question1.email, question1.Number);
        if (userExists(question1.Name, question1.email, question1.Number)) {
            ...
            users.push(user1);
            console.log(chalk.yellowBright(`Your account has been created successfullly!`));
        }
    }
        ...
        if (userExists(question2.Name, question2.email, question2.Number)) {
            ...
            while (condition2) {
                ...
                if (question4.option === "View Events details") {
                    ...
                    if (eventsList.length === 0) {
                        console.log(chalk.red(`You've not made any purchase yet as there is no event scheduled!`));
                    }
                    else {
            ...
            while (condition1) {
                ...
                if (question7.adminWork === "Create Event") {
                    ...
                    var event = new Event(question8.EventName, question8.venue, question8.timing, question8.date, question8.ticket, question8.price);
                    eventsList.push(event);
                    console.log(chalk.yellowBright(`Your event has been scheduled & published!`));
                }
                ...
                if (question7.adminWork === "Log Out") {
                    condition1 = false;
                }
                ;
            }
            ...
            console.log(chalk.red(`You are not an existing user!,Create an account first.`));
        }
    }
}
